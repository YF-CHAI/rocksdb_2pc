...
#include <iostream>//cyf add for sleep
...
void VerifyDBFromDB(std::string& truth_db_name) {
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      //usleep(1000*1000*1000);// cyf sleep for 1 second
    }
    if (FLAGS_statistics) {
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      if (thread->shared->write_rate_limiter.get() != nullptr) {
        ...
        //std::cout<<"entries_per_batch_ is:"<<entries_per_batch_<<std::endl;//cyf for testing
      ...
      if (!use_blob_db_) {
        ...
        //std::cout<<"db_bench_tool.cc line:3672 write op\n";//cyf for testing
#endif  //  ROCKSDB_LITE
      }
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (FLAGS_num_column_families > 1) {
        ...
        reset_pinnable += pin_end - pin_start;//cyf to calculate the pinnable cost
      ...
      if (s.ok()) {
        ...
        fprintf(stderr, "Get returned an error: %s\n", s.ToString().c_str());
        printf("line4189:the status code: %d,subcode: %d\n",(int)s.code(),(int)s.subcode());
        abort();
      }
